...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //  something more generic
    ...
    //Searches for a regex pattern such as word=word or digit=digit
    std::regex search("([a-zA-Z]'='[a-zA-Z]) | ([0-9]=[0-9])");
    std::smatch match;
  ...
  //Searches for an attack. Displays a message if an attack is suspected
  if (std::regex_search(sql.begin(), sql.end(), match, search) && match.size() > 1) {
      std::cout << "SQL Injection Attack is Suspected" << std::endl;
      return false;
  }
